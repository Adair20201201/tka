    ...
    public class WarningViewModel : ViewModelBase
    {
        ...
        public WarningViewModel(ConfigModel CM)
        {
            ...
            for (int i = 0; i < CM.LTM.Count; i++)
            {
                ...
                twvm.TrackNum = CM.LTM[i].TrackNumTitle; //int.Parse(CM.LTM[i].TrackID);
                TWVM.Add(twvm);
            }
        ...
        public void RemoveNotification(string notifyinfo, int trackNumber)
        {
            ...
            {
                if (_NotificationList[i].NotifyInfo.Equals(notifyinfo) && _NotificationList[i].TrackNumber == trackNumber.ToString())
                {
                    index = i;
                    break;
                }
